...
//preparation for automatically change status from "WFP" to "Assigned" after 5 days.
 foreach($dbf->fetchOrder("clients c,work_order w","w.client_id=c.id AND w.work_status ='WFP' AND w.approve_status='1'","w.id ASC","w.id,w.wo_no,c.name,c.city,c.phone_no","")as $res_workorder){
		...
		$lastdate = date("Y-m-d", strtotime('+5 days',strtotime($lastwfpDate)));
		
		$resTech =$dbf->fetchSingle("assign_tech at,technicians tc","at.tech_id=tc.id AND at.wo_no='$res_workorder[wo_no]'");
		//echo $res_workorder['wo_no'].'=='.$todate .'=='. $lastdate;exit;
		if($todate == $lastdate) {
			//echo $res_workorder['wo_no'].'=='.$todate .'=='. $lastdate;exit;
			//update work status from WFP to Assigned
			$dbf->updateTable("work_order","work_status='Assigned',updated_date=now()","id='".$res_workorder['id']."'");
			$dbf->updateTable("assign_tech","start_date='',start_time='',end_time=''","wo_no='".$res_workorder['wo_no']."'");
			$dbf->updateTable("workorder_notes","waiting_parts='2'","workorder_id='".$res_workorder['id']."' AND (user_type='admin' OR user_type='user' OR user_type='tech') AND waiting_parts !=0");
