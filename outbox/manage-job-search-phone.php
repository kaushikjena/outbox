                        ...
                        <div class="spacer"></div>
                              <?php
							   ...
							   if($_REQUEST['srchInputBoxphno']!=''){
									...
									$sch=$sch."c.phone_no IN ($searchString) OR t.contact_phone IN ($searchString)";
									if($sch !=''){
									...
									###########Search condition for search jobs################
									###########qry for search jobs################
									$qry = "SELECT w.id,w.wo_no,w.purchase_order_no, w.work_status, w.created_by,c.name, c.city,c.phone_no,c.address, st.state_name, s.service_name,cl.name as cname , CONCAT_WS(' ',t.first_name,t.last_name)as techname,at.start_date,t.contact_phone FROM state st,clients c,service s,work_order w LEFT JOIN clients as cl ON w.created_by=cl.id LEFT JOIN assign_tech at ON at.wo_no=w.wo_no LEFT JOIN technicians t ON t.id=at.tech_id WHERE ".$cond." ORDER BY w.id DESC";
