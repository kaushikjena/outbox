<?php
	...
	function getJobDetails($woid,$userid){
		global $dbf;$returnData="";
		$res_viewJobBoard=$dbf->fetchSingle("clients c,service s,work_order w","c.id=w.client_id AND w.service_id=s.id  AND w.id='".$woid."'");
		$resTech = $dbf->fetchSingle("assign_tech at,technicians tc","at.tech_id=tc.id AND at.wo_no='$res_viewJobBoard[wo_no]' AND tc.id='".$userid."'");
		//fetch tech instruction
		...
		$techInstruction = preg_replace('/[ ]{2,}|[\t]/', '  ', trim($techInstruction));
		//$notes = str_replace('\t', ' ', $res_viewJobBoard['notes']);
		$techInstruction =str_replace(array("\r\n", "\r", "\n"), '\n', $techInstruction);
		$returnData.= ', "Tech Instruction":"'.$techInstruction.'"';
		//json data for Job Description section
		//$subArray4 = "Job Description".':"'.$res_viewJobBoard['notes'].'"';
		//$notes = trim(preg_replace('/\s+/g', ' ', $res_viewJobBoard['notes']));
		$notes = preg_replace('/[ ]{2,}|[\t]/', '  ', trim($res_viewJobBoard['notes']));
		//$notes = str_replace('\t', ' ', $res_viewJobBoard['notes']);
		...
		$res_woservice = $dbf->fetch("equipment e,work_type wt,workorder_service ws","e.id=ws.equipment AND wt.id=ws.work_type AND ws.workorder_id='".$woid."'");
		...
		$resNotes=$dbf->fetchOrder("workorder_notes","workorder_id='".$woid."' AND user_type='tech' AND customer_attempt=0 AND waiting_parts NOT IN(1,2)","created_date DESC");
		$dataArray1 = array();
		foreach($resNotes as $resn){
			 if($resn['user_type']=='tech'){
				 $unameTech = $dbf->strRecordID("technicians","first_name, middle_name,last_name","id='".$resn['user_id']."'");
		...
		//json data for tech customer contact
		$resNotes=$dbf->fetchOrder("workorder_notes","workorder_id='".$woid."' AND user_type='tech' AND customer_attempt <>0","created_date DESC");
		$dataArray2 = array();
		foreach($resNotes as $resn){
			 if($resn['user_type']=='tech'){
				 $unameTech = $dbf->strRecordID("technicians","first_name, middle_name,last_name","id='".$resn['user_id']."'");
			...
			$by = "#". $resn['customer_attempt']." attempt By ".$uname ." on " . date("d-M-Y g:i A",strtotime($resn['created_date'])). " for #" . $res_viewJobBoard['wo_no'];
			$subres = array("Note" => $resn['wo_notes'],"By" => $by);
			array_push($dataArray2,$subres); 
		}
		$returnData.= ', "Customer Contacts":'.json_encode($dataArray2);
		//json data for waiting for parts
		$waitingnotes=$dbf->fetchSingle("workorder_notes","workorder_id='".$woid."' AND (user_type='admin' OR user_type='tech') AND waiting_parts!=0");
		$dataArray3 = array();
		if(!empty($waitingnotes)){
			 if($waitingnotes['user_type']=='tech'){
				 $unameTech = $dbf->strRecordID("technicians","first_name, middle_name,last_name","id='".$waitingnotes['user_id']."'");
				 $uname = $unameTech['first_name'].' '.$unameTech['middle_name'].' '.$unameTech['last_name'];
			 }elseif($waitingnotes['user_type']=='admin'){
				 $uname = $dbf->getDataFromTable("admin","name","id='".$waitingnotes['user_id']."'");
			 }
			$by = "WFP Note: By ".$uname ." on " . date("d-M-Y g:i A",strtotime($waitingnotes['created_date'])). " for #" . $res_viewJobBoard['wo_no'];
			$subres = array("Note" => $waitingnotes['wo_notes'],"By" => $by,"Waiting Status" => $waitingnotes['waiting_parts']);
			array_push($dataArray3,$subres); 
		}
		$returnData.= ', "Waiting For Parts":'.json_encode($dataArray3);
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="login" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		//Store Gcm id only for android devices.
			...
			if($num>0){
				...
				if($res_login['status']=='1'){
					...
					if($res_login['user_type']=='tech'){
						#################INSERT INTO TECHNICIAN TOKEN TABLE######################
						$tech_data=$dbf->fetchSingle("technician_token","device_token='".$device_token."' AND device_type='".$device_type."' LIMIT 1");
						if(!$tech_data){
							//insert into trainer token table
							$dbf->insertSet("technician_token","tech_id='".$res_login['id']."',device_token='".$device_token."',gcm_id='".$gcm_id."',device_type='".$device_type."',created_date=now()");
						}else{								
							$dbf->updateTable("technician_token","trainer_id='".$res_login['id']."',device_token='".$device_token."',gcm_id='".$gcm_id."',device_type='".$device_type."',created_date=now()","device_token='".$device_token."'");
						}
						#################INSERT INTO TECHNICIAN TOKEN TABLE######################
						##################Calendar data for techs###########################
						$dataArray=array();
						$resArray = $dbf->fetchOrder("assign_tech at,clients c,work_order wo"," at.wo_no=wo.wo_no AND wo.client_id=c.id AND at.tech_id='".$res_login['id']."'","wo.id","at.assign_date,at.start_date, at.start_time, wo.id, wo.work_status, wo.purchase_order_no,at.wo_no,c.name,c.city,c.latitude,c.longitude");
						foreach($resArray as $resevent) {
							$eventdt= ($resevent['start_date'] == '0000-00-00')?$resevent['assign_date']: $resevent['start_date'];
							if($resevent['start_time']<>''){$eventdt.= date(" H:i:s",strtotime($resevent['start_time']));}else{
								$eventdt.=" 00:00:00";
							}
							$title = $resevent['wo_no'];
							$description = addslashes($resevent['purchase_order_no']).','.addslashes($resevent['name']).','.addslashes($resevent['city']);
							$subArray = array("Title" => $title, "WorkStatus" => $resevent['work_status'],"Description" => $description,"StartDate" => $eventdt,"Latitude" => $resevent['latitude'],"Longitude" => $resevent['longitude']);
			 				array_push($dataArray,$subArray);
						}
						##################Calendar data for techs###########################
						echo '{"success":"true","userid":"'.$res_login['id'].'","username":"'.$techname.'","CalendarData":'.json_encode($dataArray).'}';exit;
					}
				}else{
	...
	######################################### LOGOUT WEBSERVICE #########################################
	#####################################################################################################
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="logout" && $_SERVER['REQUEST_METHOD']=='GET'){
		$jsonData=json_decode($_REQUEST['data']);
		$tech_id = $jsonData->userid;
		$device_token=$jsonData->device_token;
		$device_type=$jsonData->device_type;
		if($tech_id=="" || $device_token=="" || $device_type==""){
			echo '{"success":"false","err_msg":"Sorry ! Invalid credentials."}';exit;
		}else{
			$tech_data=$dbf->fetchSingle("technician_token","tech_id='".$tech_id."' AND device_token='".$device_token."' AND device_type='".$device_type."' LIMIT 1");
			if($tech_data){
				$dbf->deleteFromTable("technician_token","id='".$tech_data['id']."'");
				echo '{"success":"true","userid":"'.$tech_id.'","suc_msg":"Logout Successfully."}';exit;
			}else{
				echo '{"success":"false","err_msg":"Sorry ! Invalid credentials."}';exit;
			}
		}	
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="getCalendarData" && $_SERVER['REQUEST_METHOD']=='GET'){
		$jsonData=json_decode($_REQUEST['data']);
		//check user type tech in database
		$resType =$dbf->getDataFromTable("technicians","user_type","id='".$jsonData->userid."'");
		//return data to iphone url
		if($resType=='tech'){
			##################Calendar data for techs###########################
			$dataArray=array();
			$resArray = $dbf->fetchOrder("assign_tech at,clients c,work_order wo"," at.wo_no=wo.wo_no AND wo.client_id=c.id AND at.tech_id='".$jsonData->userid."'","wo.id","at.assign_date, at.start_date, at.start_time, wo.id, wo.purchase_order_no, wo.work_status, at.wo_no, c.name, c.city, c.latitude, c.longitude");
			foreach($resArray as $resevent) {
				$eventdt= ($resevent['start_date'] == '0000-00-00')?$resevent['assign_date']: $resevent['start_date'];
				if($resevent['start_time']<>''){$eventdt.= date(" H:i:s",strtotime($resevent['start_time']));}else{
					$eventdt.=" 00:00:00";
			...
			##################Calendar data for techs###########################
			echo '{"success":"true","data":'.json_encode($dataArray).'}';exit;
		}else{
			echo '{"success":"false"}';exit;
		}
	}
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="scheduled" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		if($resType=='tech'){
			...
			foreach($resultArray as $res_JobBoard){
				$schdate = $res_JobBoard['start_date'];
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="jobdetails" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		$getResultData =  getJobDetails($woid,$userid);//calling jobdetails function
		$returnData = '{'.$getResultData.'}';
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="addnote" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		$userid =$jsonData->userid;
		$type =$jsonData->type;
		$waitingstatus =$jsonData->waitingstatus;
		if($type=="note"){
			$techNotes=mysql_real_escape_string($jsonData->note);
			$string="workorder_id='".$woid."', user_type='tech', user_id='".$userid."', wo_notes='".$techNotes."',created_date=now()";
			$insid = $dbf->insertSet("workorder_notes",$string);
		}elseif($type=="cc"){
			//fetch notes from work order notes table
			$resNotes=$dbf->fetchOrder("workorder_notes","workorder_id='".$woid."' AND user_type='tech' AND customer_attempt <>0","created_date DESC");
			$attempt = count($resNotes)+1;
			$techNotes="This is the #".$attempt." attempt to customer";
			$string="workorder_id='".$woid."', user_type='tech', user_id='".$userid."', wo_notes='".$techNotes."', customer_attempt='".$attempt."', created_date=now()";
			$insid =$dbf->insertSet("workorder_notes",$string);
			/********Admin Email************/
			$res_wo=$dbf->strRecordID("work_order","wo_no,client_id,purchase_order_no,created_by","id='".$woid."'");
			$purchaseOrder = $res_wo['purchase_order_no'];
			//get client name
			...
			//get admin details
			$res_admin=$dbf->getDataFromTable("admin","name","id=1");
			$AdminName=$res_admin['name'];
			//$to=$res_admin['email'];
			$admin_email = $dbf->strRecordID("admin_email_notification","to_email,status","id='7'");
			$to=$admin_email['to_email'];
			//get email template
			$admin_notification = $admin_email['status'];
			if($admin_notification==1){
				...
				$tech=$dbf->strRecordID("technicians","first_name,middle_name,last_name","id='".$userid."'");
			...
			//fetch notes from work order notes table
			$techNotes=mysql_real_escape_string($jsonData->note);
			$cntrows =$dbf->strRecordID("workorder_notes","id","workorder_id='".$woid."' AND waiting_parts !='0'");
			if($cntrows['id']==0){
				$string="workorder_id='".$woid."', user_type='tech', user_id='".$userid."', wo_notes='".$techNotes."',waiting_parts='".$waitingstatus."',created_date=now()";
				$insid =$dbf->insertSet("workorder_notes",$string);
				//update work order table
				$dbf->updateTable("work_order","work_status='WFP',updated_date=now()","id='".$woid."'");
				//email to admin
				$res_wo=$dbf->strRecordID("work_order","wo_no,client_id,purchase_order_no,created_by","id='".$woid."'");
				$purchaseOrder = $res_wo['purchase_order_no'];
				$admin_email = $dbf->strRecordID("admin_email_notification","to_email,status","id='8'");
				$admin_notification = $admin_email['status'];
				if($admin_notification==1){
					//get client name
					$clientName =$dbf->getDataFromTable("clients","name","id='$res_wo[created_by]'");
					...
					$from=$res_template['from_email'];
					$fromname = $res_template['from_name'];
					$subject=$res_template['subject']." == ".$clientName." == ".$res_wo['wo_no'];
					$input=$res_template['message'];
					$body =str_replace(array('%AdminName%','%CustomerName%','%WorkOrder%','%PurchaseOrder%','%ClientName%'),array($AdminName,$customerName,$res_wo['wo_no'],$purchaseOrder,$clientName),$input);
					$headers = "MIME-Version: 1.0\n";
					$headers .= "Content-type: text/html; charset=UTF-8\n";
					$headers .= "From:".$fromname." <".$from.">\n";
					//echo $body;exit;
				...
				$string="workorder_id='".$woid."', wo_notes='".$techNotes."',waiting_parts='".$waitingstatus."',created_date=now()";
				$dbf->updateTable("workorder_notes",$string,"id='".$cntrows['id']."'");
				$insid=$cntrows['id'];
			}
		}
		if($insid){
			$getResultData =  getJobDetails($woid,$userid);//calling jobdetails function
			echo '{"success":"true",'.$getResultData.'}';exit;
		}else{
			echo '{"success":"false"}';exit;
		}
	}
	############CONDITION FOR SERVICE CALL#############
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="wfpboard" && $_SERVER['REQUEST_METHOD']=='GET'){
		//print_r(json_decode($_REQUEST['data']));exit;
		$jsonData=json_decode($_REQUEST['data']);
		$resultArray =$dbf->fetchOrder("state st,clients c,service s,technicians t,assign_tech at,work_order w","st.state_code=c.state AND c.id=w.client_id AND w.service_id=s.id AND w.work_status IN('WFP','On Hold') AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND t.id='".$jsonData->userid."'","w.id DESC","st.state_name,c.name,c.city,w.id,w.wo_no,w.work_status,s.service_name,at.start_date,at.start_time","");
		$dataArray = array();
		$headerArray =array("woid","wo#","CustomerName","ServiceName","WorkStatus","DeliveryCity","DeliveryState","Scheduled Date & Time");
		foreach($resultArray as $res_JobBoard){
			//echo $res_JobBoard['wo_no']."\n";
			$workstatus = $res_JobBoard['work_status']?$res_JobBoard['work_status']:"Open";
			$startdatetime = ($res_JobBoard['start_date']<>'0000-00-00')? date("M/d/Y",strtotime($res_JobBoard['start_date'])).' '.$res_JobBoard['start_time']:""; 
			$subArray = array("woid" => $res_JobBoard['id'],"wo#" => $res_JobBoard['wo_no'],"CustomerName" => $res_JobBoard['name'],"ServiceName" => $res_JobBoard['service_name'],"WorkStatus" => $workstatus,"DeliveryCity" => $res_JobBoard['city'],"DeliveryState" => $res_JobBoard['state_name'],"Scheduled Date & Time" => $startdatetime);
			 array_push($dataArray,$subArray);
		}
		echo '{"header":'.json_encode($headerArray).',"data":'.json_encode($dataArray).'}';
	}
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="getschedule" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		$resTechName = ($resTech<>'')? (($resTech['middle_name']<>'')?($resTech['first_name'].' '.$resTech['middle_name'].' '.$resTech['last_name']):($resTech['first_name'].' '.$resTech['last_name'])):"";
		$startDate = ($resTech['start_date']<>'0000-00-00')?$resTech['start_date']:'';
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="submitschedule" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		$asgnStartDate= ($jsonData->StartDate !="")?date("Y-m-d",strtotime($jsonData->StartDate)):"";
		...
		if($num>0){
			...
			//$cid = $dbf->getDataFromTable("work_order","client_id","wo_no='$WorkNo'");
			//$clientName=$dbf->getDataFromTable("clients","name","id='$cid'");
			...
			$clientName = $clientName ? $clientName :"COD";
			########get client name for subject line###############
			$purchaseOrder = $dbf->getDataFromTable("work_order","purchase_order_no","wo_no='$WorkNo'");
			...
			$from=$tech['email'];
			$TechName=$tech['first_name'].'&nbsp;'.$tech['middle_name'].'&nbsp;'.$tech['last_name'];
			$body=str_replace(array('%AdminName%','%TechName%','%StartDate%','%StartTime%','%EndTime%','%WorkOrders%','%PurchaseOrder%', '%ClientName%'),array($AdminName,$TechName,$asgnStartDate,$asgnStartTime,$asgnEndTime,$WorkNo,$purchaseOrder, $clientName),$input);
			$headers = "MIME-Version: 1.0\n";
			$headers .= "Content-type: text/html; charset=UTF-8\n";
			...
			$emailbody=str_replace(array('%ClientName%','%TechName%','%StartDate%','%StartTime%','%EndTime%','%WorkOrders%','%PurchaseOrders%'),array($toclientname,$Techname,$asgnStartDate,$asgnStartTime,$asgnEndTime,$WorkNo,$purchaseOrder),$input1);
			//echo $emailbody;exit;
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="submitWorkstatus"){
		...
		fwrite($fp,$str);
		fclose($fp);*/
		$arrivalDate=($jsonData->ArrivalDate !="")?date("Y-m-d",strtotime($jsonData->ArrivalDate)):"";
		$fileWorkImageAmount = $jsonData->fileWorkImageAmount;
		$fileWorkDocAmount = $jsonData->fileWorkDocAmount;
		$partStatus = $jsonData->partStatus;
		$partNeededComments = $_POST['partNeededComments'];
		//check if the user exist or work status is blank
		$resType =$dbf->getDataFromTable("technicians","user_type","id='".$jsonData->userid."'");
		if($resType=='tech' && $jsonData->WorkStatus !=""){
			//update work_order table
			 $string="work_status='".$jsonData->WorkStatus."',parts_status='".$partStatus."',parts_needed_comments='".$partNeededComments."'";
			 $dbf->updateTable("work_order",$string,"wo_no='".$jsonData->wono."'");
			 
			 $techNotes= mysql_real_escape_string($_POST['TechNotes']);
			...
			if($fileWorkImageAmount >0){
				for($i=1;$i<=$fileWorkImageAmount;$i++){
					...
					$file_name=$jsonData->wono.'_'.date('dhihis')."_".$fileWorkImage.".jpeg";
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="getWorkStatus" && $_SERVER['REQUEST_METHOD']=='GET'){
		//print_r(json_decode($_REQUEST['data']));exit;
		$jsonData=json_decode($_REQUEST['data']);
		$userid = $jsonData->userid;
		$woid = $jsonData->woid;
		$workOrderArr = $dbf->strRecordID("work_order","wo_no, work_status, parts_status, parts_needed_comments","id='".$woid."'");
		$wono = $workOrderArr['wo_no'];
		$resultArray =$dbf->fetchOrder("workorder_doc","workorder_id='".$woid."' AND created_user='".$userid."' AND user_type='tech'","id DESC","wo_document","");
		$resultSerialArray =$dbf->fetchOrder("work_order_serial_no","wo_no='".$wono."' AND tech_id='".$userid."'","id DESC","serial_no","");
		$dataImgArray = array();
		foreach($resultArray as $resdoc){
			if (strpos($resdoc['wo_document'], 'fileWorkImage') !== false) {
				$wodocpath = $baseUrl."workorder_doc/".$resdoc['wo_document'];
			 	array_push($dataImgArray,$wodocpath);
			}
		}
		$dataDocArray = array();
		foreach($resultArray as $resdoc){
			if (strpos($resdoc['wo_document'], 'fileWorkDoc') !== false) {
				$wodocpath = $baseUrl."workorder_doc/".$resdoc['wo_document'];
			 	array_push($dataDocArray,$wodocpath);
			}
		}
		$dataSignArray = array();
		foreach($resultArray as $resdoc){
			if (strpos($resdoc['wo_document'], 'fileSignature') !== false) {
				$wodocpath = $baseUrl."workorder_doc/".$resdoc['wo_document'];
			 	array_push($dataSignArray,$wodocpath);
			}
		}
		$dataSerialArray = array();
		foreach($resultSerialArray as $resdoc){
			array_push($dataSerialArray,$resdoc['serial_no']);
		}
		$returnString =  '{"fileWorkImage":'.json_encode($dataImgArray).',"fileWorkDoc":'.json_encode($dataDocArray).',"fileSignature":'.json_encode($dataSignArray).',"MachineSerialNo":'.json_encode($dataSerialArray).',"workStatus":"'.$workOrderArr['work_status'].'","partStatus":"'.$workOrderArr['parts_status'].'","partNeededComments":"'.$workOrderArr['parts_needed_comments'].'"}';
		echo $returnString;
		//print_r(json_decode($returnString));
	}
	############CONDITION FOR TECH JOB REPORTS#############
