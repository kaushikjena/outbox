	   ...
	   //Email send to customer as the order is posted in the system.
		...
		if($to){
			if(@mail($to,$subject,$body,$headers)){
				...
				$adminNotes=$subject." send to customer from this work order.";
		...
		$("#loader").hide();
		 if(res=='1'){
			window.location.href="manage-job-board"; 
			 ...
			 $.fancybox("<span><b>Sorry! This order is already assigned.</b></span>",{centerOnScroll:true,hideOnOverlayClick:false});			
		 }else{			
			 $.fancybox(res,{centerOnScroll:true,hideOnOverlayClick:false});			
		 }
	 	});
