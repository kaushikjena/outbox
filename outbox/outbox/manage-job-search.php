...
<body>
	<form name="frmRedirect" id="frmRedirect" action="" method="post"> 
        ...
        <input type="hidden" name="hidk" id="hidk" value="0"/>
    </form>
    <div id="maindiv">
        <!-------------header--------------->
                              ...
                              <?php
							   ###################SEARCH PHONE NUMBERS AND WO AND PO AND Client Name###########
								...
								if($_REQUEST['srchInputBox']!=''){
									...
									###########Search condition for search jobs################
									###########qry for search jobs#############################
									$qry = "SELECT w.id,w.wo_no,w.purchase_order_no, w.work_status, w.created_by,c.name, c.city,c.phone_no,c.address, st.state_name, s.service_name,cl.name as cname , CONCAT_WS(' ',t.first_name,t.last_name)as techname,at.start_date FROM state st,clients c,service s,work_order w LEFT JOIN clients as cl ON w.created_by=cl.id LEFT JOIN assign_tech at ON at.wo_no=w.wo_no LEFT JOIN technicians t ON at.tech_id=t.id WHERE ".$cond." ORDER BY w.id DESC";
                                     ...
                                     <?php 
										foreach($resArray as $key=>$res_JobBoard) {
											...
											if($res_JobBoard['work_status']=='Open'){
												...
												if($res_JobBoard['work_status']=='Assigned' && ($res_JobBoard['start_date']=='0000-00-00' || $res_JobBoard['start_date']=='')){
													$board ="Assigned";
