...
<?php }else if(isset($_REQUEST['choice'])&& $_REQUEST['choice']=="data_update"){
	...
	$purchaseOrder=$dbf->getDataFromTable("work_order","purchase_order_no","id='".$_REQUEST['woid']."'");
	...
	$clientName =$dbf->getDataFromTable("work_order w,clients c","c.name","w.created_by=c.id AND w.id='".$_REQUEST['woid']."'");
	...
	if($num>0){
		...
		$admin_email = $dbf->getDataFromTable("admin_email_notification","to_email","id=4");
		if($admin_notification==1){
			...
			$email_body=str_replace(array('%AdminName%','%TechName%','%StartDate%','%StartTime%','%EndTime%','%WorkOrders%','%PurchaseOrder%','%ClientName%'),array($AdminName,$TechName,$_REQUEST['StartDate'],$_REQUEST['StartTime'],$_REQUEST['EndTime'],$WorkNo,$purchaseOrder,$clientName),$input);
			$headers = "MIME-Version: 1.0\n";
			$headers .= "Content-type: text/html; charset=UTF-8\n";
			$headers .= "From:".$TechName." <".$from.">\n";
			//echo $to.'=='.$subject.'=='.$email_body;exit;
		...
		$emailbody=str_replace(array('%ClientName%','%TechName%','%StartDate%','%StartTime%','%EndTime%','%WorkOrders%','%PurchaseOrders%'),array($toclientname,$Techname,$_REQUEST['StartDate'],$_REQUEST['StartTime'],$_REQUEST['EndTime'],$WorkNo,$purchaseOrder),$input1);
		//echo $emailbody;exit;
