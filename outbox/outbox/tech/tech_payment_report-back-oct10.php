...
function Search_Records(){
	document.SrchFrm.action="tech_payment_report";
...
function ClearFields(){
	...
	window.location.href="tech_payment_report";
}
//for exporting,print,pdf,word
function print_doc(val,page){
 if(val=='word'){
	 document.SrchFrm.action="tech_payment_report_word";
	 document.SrchFrm.submit();
 }else if(val=='excell'){
	 document.SrchFrm.action="tech_payment_report_excell";
	 document.SrchFrm.submit(); 
 }else if(val=='pdf'){
	 document.SrchFrm.action="tech_payment_report_pdf";
	 document.SrchFrm.submit(); 
 }else if(val=='print'){
	document.SrchFrm.action="tech_payment_report_print?page="+page;
         ...
         <div id="contentdiv">
                ...
                <div class="rightcolumjobboard">
            		<div class="rightcoluminner">
                        <div class="headerbg">
                            <div style="float:left;;">Total Payment Report</div>
                        ...
                        <div id="contenttable">
                              ...
                              <div  class="formtextaddsrch" align="center">Customer</div>
                               ...
                               <div  class="formtextaddsrch" align="center">Client</div>
                                ...
                                <select name="srchClient" id="srchClient" class="selectboxsrch">
                                    ...
                                    <?php foreach($dbf->fetchOrder("work_order_bill wb,clients cl","wb.created_by=cl.id AND wb.created_by<>'0' AND wb.payment_status='Completed'","cl.id DESC","","cl.name")as $client){ ?>
                          ...
                          <?php
							   ...
							   //echo $sch;exit;
							   if($sch!=''){
								 $cond="st.state_code=c.state AND wb.client_id=c.id AND wb.payment_status='Completed' AND wb.tech_id=t.id AND wo.wo_no=wb.wo_no AND wo.service_id=s.id AND wb.tech_id='$_SESSION[userid]' AND ".$sch;
								  // echo $cond;exit;
							   }
							   elseif($sch==''){
								 $cond="st.state_code=c.state AND wb.client_id=c.id AND wb.payment_status='Completed' AND wb.tech_id=t.id AND wo.wo_no=wb.wo_no AND wo.service_id=s.id AND wb.tech_id='$_SESSION[userid]'";
							   }
							   //print $cond;
							   //Pagination 
                                    ...
                                    <tbody>
									 ...
									 ?>
										...
										?>   
                                            ...
                                            <td data-title="Client"><?php echo $res_jobPayment['state_name'];?></td>
                                            <td data-title="Payment Status"><b><?php echo $res_jobPayment['payment_status'];?></b></td>
                           ...
                           <?php }else{?>
                            ...
                            <?php }?>
                            <div  align="center"><?php if($num>0) { echo $dbf->Pages($num,$perpage,"tech_payment_report.php?srchCust=$_REQUEST[srchCust]&srchClient=$_REQUEST[srchClient]&srchService=$_REQUEST[srchService]&FromDate=$_REQUEST[FromDate]&ToDate=$_REQUEST[ToDate]&");}?></div>
                          </div>
                        </div>
                        <div class="spacer"></div>
                    </div>
