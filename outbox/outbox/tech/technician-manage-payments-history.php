                        ...
                        <div id="contenttable">
                            ...
                            <form name="SrchFrm" id="SrchFrm" action="" method="post">
							  ...
							  function getStartAndEndDate($week, $year) {
								...
								$current_year = "2013";//current year
								...
								foreach($week_array as $k=>$w){
									...
									foreach($dbf->fetchOrder("work_order w,work_order_tech_bill wb","w.wo_no=wb.wo_no AND wb.payment_status='Completed' AND wb.tech_id='$_SESSION[userid]' AND wb.payment_date BETWEEN $date_range","","wb.wo_no","")as $res){
							  ...
							  ###########################################################################
							   ...
							   //echo $sch;exit;
							   if($sch!=''){
								 $cond="c.id=wo.client_id AND wo.wo_no=wb.wo_no AND wo.service_id=s.id AND wb.tech_id='$_SESSION[userid]' AND ".$sch;
								  // echo $cond;exit;
							   }
							   elseif($sch==''){
								 $cond="c.id=wo.client_id AND wo.wo_no=wb.wo_no AND wo.service_id=s.id AND wb.tech_id='$_SESSION[userid]'";
							   }
							  ?>
                              <!-----Table area start------->
                                ...
                                foreach($mainWoArray as $key=>$warray){
                                   ...
                                   if(!empty($warray)){
									...
									$count=$dbf->countRows("clients c,service s,work_order wo,work_order_tech_bill wb",$cond." AND FIND_IN_SET(wb.wo_no,'$implode_workorders')");
									 if($count>0){
                                ?>
								...
								<?php 
                                    $resPayments=array();$grandtotal=0;
                                    $resPayments=$dbf->fetchOrder("clients c,service s,work_order wo,work_order_tech_bill wb",$cond." AND FIND_IN_SET(wb.wo_no,'$implode_workorders')","","c.name,s.service_name,wo.wo_no,wo.purchase_order_no,wo.service_id,wo.id as woid,wo.work_status,wb.payment_date,wb.subtotal,wb.id","");
                                    //group by tech loop
                                    foreach($resPayments as $k=>$techpayhistory){
										$grandtotal = $grandtotal+ $techpayhistory['subtotal'];
										
										//fetch work type, model and total price of work order
										$workTypeArray =array(); $modelArray =array();
										$res_woservice = $dbf->fetch("equipment e,work_type wt,workorder_service ws","e.id=ws.equipment AND wt.id=ws.work_type AND ws.service_id='$techpayhistory[service_id]' AND ws.workorder_id='$techpayhistory[woid]'");
								...
								?>
                                  <tr>
                                    ...
                                    <td data-title="Service"><?php echo $techpayhistory['service_name'];?></td>
                                    <td data-title="WorkType"><?php echo $workType;?></td>
                                    <td data-title="Model"><?php echo $model;?></td>
                                    <td data-title="CustomerName"><?php echo $techpayhistory['name'];?></td>
                                    <td data-title="WO#"><?php echo $techpayhistory['wo_no'];?></td>
                                    <td data-title="PO#"><?php echo $techpayhistory['purchase_order_no'];?></td>
                                   	<td data-title="Subtotal">$ <?php echo $techpayhistory['subtotal'];?></td>
                                    <td data-title="Action" class="coltext"><a href="view-technician-payment-history?id=<?php echo $techpayhistory['id']?>"><img src="../images/view.png" title="View" alt="View"/></a></td>
                               	 </tr>
								   <?php 
