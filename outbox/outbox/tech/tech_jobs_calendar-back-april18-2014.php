                                ...
                                <div style="float:right;width:54%;border:solid 1px #999;">
                                    ...
                                    <script type="text/javascript" src="../js/jquery.calendario.js"></script>
                                        <script type="text/javascript">	
                                            ...
                                            <?php 
                                                $resArrayDate=array();
												foreach($dbf->fetchOrder("technicians t,assign_tech at","t.id=at.tech_id AND t.id='$_SESSION[userid]'","","at.wo_no,at.start_date","") as $val){
												   array_push($resArrayDate,$val['start_date']);
												 }
												$resArrayDate= array_unique($resArrayDate);
												foreach($resArrayDate as $v){
												$resArray = $dbf->fetchOrder("assign_tech at,clients c,work_order wo"," at.wo_no=wo.wo_no AND wo.client_id=c.id AND at.tech_id='$_SESSION[userid]' AND at.start_date='$v'","","at.start_date,wo.id,at.wo_no,c.name,c.city");
												$string='';
												if(count($resArray) > 1){
													foreach($resArray as $resevent) {
														$eventdt= date("m-d-Y",strtotime($resevent['start_date']));
														$linkhref="tech-view-job-board?id=$resevent[id]&src=cal"; $target = "_self";
                                            ...
                                            $(function() {
                                                ...
                                                function showEvents( $contentEl, dateProperties ) {
                                                    hideEvents();
                                                    var $events = $( '<div id="custom-content-reveal" class="custom-content-reveal"><h4>Events for ' + dateProperties.monthname + ' ' + dateProperties.day + ', ' + dateProperties.year + '</h4><span>Scheduled Works</span></div>' ),
