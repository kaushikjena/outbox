                    ...
                    <div id="contenttable">
                        ...
                        <div class="spacer"></div>
                         <form action="" name="frmEmailTech" id="frmEmailTech" method="post" enctype="multipart/form-data">
                            ...
                            <div class="spacer"></div>
                            <div  class="formtextadd">Comments:</div>
                            <div  class="textboxc" style="width:80%;">
                                <textarea class="textarea" name="techmessage"  id="techmessage"></textarea>
                            </div>
                            <div class="spacer"></div>
                            <div align="center">
...
<?php }else if(isset($_REQUEST['choice'])&& $_REQUEST['choice']=="send_email"){
	...
	$cid=$dbf->getDataFromTable("work_order","client_id","id='$woid'");
	$clientName=$dbf->getDataFromTable("clients","name","id='$cid'");
	$purchaseOrder=$dbf->getDataFromTable("work_order","purchase_order_no","id='$woid'");
	...
	//Email Sending Starts here
		...
		if($admin_notification ==1){
			...
			$subject=addslashes($_REQUEST['subject'])."==".$clientName;
			$message=$_REQUEST['message'];
			$TechMessage=$_REQUEST['techmessage'];
			...
			//Email Sending Starts here
			$body=str_replace(array('%Administrator%','%WorkOrder%','%PurchaseOrder%','%TechName%','%TechComments%'),array($toAdminName,$wono,$purchaseOrder,$toTechName,$TechMessage),$message);
			//echo $body;exit;
