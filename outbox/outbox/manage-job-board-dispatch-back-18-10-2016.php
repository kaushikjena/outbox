            		...
            		<div class="rightcoluminner">
                    	<?php
						   ...
						   if($sch!=''){
							 ...
							 $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND (w.work_status='Dispatched' OR w.work_status='Assigned' OR w.work_status='Scheduled') AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND at.start_date=CURDATE()";
						   }
						   //condition for users
						   if($implode_clients <>''){
                                ...
                                <table id="no-more-tables" class="draggable">
                                    ...
                                    <tbody>
                                     <tr style="background-color:#f9f9f9;">
                              		...
                              		<?php 
										//group array
										$resGrArray=$dbf->fetchOrder("state st,clients c,service s,technicians t,assign_tech at,work_order w",$cond,"c.state ASC","st.state_name,c.state,count(w.id) as numres","c.state");
										//group by state loop
								    ...
								    <?php
									$resArray=$dbf->fetchOrder("state st,clients c,service s,technicians t,assign_tech at,work_order w","c.state='".$sgRes['state']."' AND " .$cond,$orderby." ".$orderType,"st.state_name, c.name, c.city, w.id, w.wo_no, w.purchase_order_no, w.service_id,w.work_status,w.created_by, s.service_name, t.first_name, t.middle_name, t.last_name, at.start_date, at.start_time",$groupby);
									//print "<pre>";
									//print_r($resArray);
