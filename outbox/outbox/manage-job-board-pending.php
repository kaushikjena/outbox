...
function ClearFields(){
	$('#FromDate').val("");
	$('#ToDate').val("");
	$.post("unset-session.php",{"src":"pending"},function(res){
		$("#schaction").val("");
		$("#SrchFrm").submit();
	});
}
...
function edit_filter(){
	window.location.href="edit-filter-search-pending";
}
/*********Function to show create job************/
...
/**********Send Notification to assigned Tech*********/
	...
	<form name="frmRedirect" id="frmRedirect" action="" method="post"> 
        ...
        <input type="hidden" name="src" value="pending"/>
                ...
                <div class="rightcolumjobboard">
            		<div class="rightcoluminner">
                    	<?php
							...
							$insert= "user_type='$_SESSION[usertype]',user_id='$_SESSION[userid]', page_name='pendingboard'";
						   ...
						   if($_SESSION['requesth']['schaction'] =='filtersch'){
							   $count_filter_search =$dbf->countRows("filter_search","user_type='$_SESSION[usertype]' AND user_id='$_SESSION[userid]' AND page_name='pendingboard'");
							   if($count_filter_search){
									$dbf->updateTable("filter_search",$insert.",updated_date=now()","user_type='$_SESSION[usertype]' AND user_id='$_SESSION[userid]'  AND page_name='pendingboard'");
							   }else{
						   ...
						   if($sch!=''){
							 $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND ".$sch;
							  if($srchStatus==''){$cond.=" AND w.work_status ='Pending'";}
						   }elseif($sch==''){
							 $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND w.work_status ='Pending' AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id";
						   }
						   //condition for users
						   if($implode_clients <>''){
						  ...
						  ?>
                        <div class="headerbg">
                        	<div style="float:left; width:30%;">On Pending Board</div>
                        ...
                        <div id="contenttable">
                            ...
                            <input type="hidden" name="page" value="PendingBoard"/>
                               <div align="center">
