...
<?php }else if(isset($_REQUEST['choice'])&& $_REQUEST['choice']=="send_email"){
	...
	if((strpos($message,'CarrierCompany') &&  strpos($message,'TrackingNo'))!== false){
		...
		if($resparts=='twodays'){
			$partsArrive=date("d-M-Y",strtotime("+2 days"));
		}else if($resparts=='threedays'){
			$partsArrive=date("d-M-Y",strtotime("+3 days"));
		}else if($resparts=='fourdays'){
			$partsArrive=date("d-M-Y",strtotime("+4 days"));	
		}else if($resparts=='Partsarrive'){
			$partsArrive=date("d-M-Y");
		}else{
			$partsArrive="";
		}
		###############################################################################################
	...
	if(@mail($to,$subject,$body,$headers)){
		...
		$adminNotes="Notification send to tech from this work order.";
