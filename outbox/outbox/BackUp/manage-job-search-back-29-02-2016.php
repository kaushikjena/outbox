         ...
         <div id="contentdiv">
                ...
                <div class="rightcolumjobboard">
            		<div class="rightcoluminner">
                        ...
                        <div class="spacer"></div>
                              <?php
							   ###################SEARCH PHONE NUMBERS AND WO AND PO AND Client Name###########
							    $sch="";
								if($_REQUEST['srchInputBox']!=''){
									...
									$sch=$sch."w.wo_no IN ($searchString) OR w.purchase_order_no IN ($searchString) OR c.name IN ($searchString)";
									if($sch !=''){
							   ...
							   if($_REQUEST['srchInputBoxphno']!=''){
									...
									###########Search condition for search jobs################
									###########qry for search jobs################
									$qry = "SELECT w.id,w.wo_no,w.purchase_order_no, w.work_status, w.created_by,c.name, c.city,c.phone_no,c.address, st.state_name, s.service_name,cl.name as cname , temp.techname,temp.start_date,temp.contact_phone FROM state st,clients c,service s,work_order w LEFT JOIN clients as cl ON w.created_by=cl.id,work_order wo LEFT JOIN (select at.wo_no,at.start_date,t.contact_phone,CONCAT_WS(' ',t.first_name,t.last_name)as techname from assign_tech at,technicians t where at.tech_id=t.id) as temp ON temp.wo_no=wo.wo_no WHERE wo.wo_no=w.wo_no AND ".$cond." ORDER BY w.id DESC";
							   		$resArray=$dbf->simpleQuery($qry);
								   //print "<pre>";//print_r($resArray);
								   ###########qry for search jobs################
								}
