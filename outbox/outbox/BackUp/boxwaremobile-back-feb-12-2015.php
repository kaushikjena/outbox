<?php
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="scheduled" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		foreach($resultArray as $res_JobBoard){
			$subArray = array("woid" => $res_JobBoard['id'],"wo#" => $res_JobBoard['wo_no'],"ServiceName" => $res_JobBoard['service_name'],"CustomerName" => $res_JobBoard['name'],"Latitude" => $res_JobBoard['latitude'],"Longitude" => $res_JobBoard['longitude'],"ScheduledDate" => $res_JobBoard['start_date']);
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="servicecall" && $_SERVER['REQUEST_METHOD']=='GET'){
		//print_r(json_decode($_REQUEST['data']));exit;
		$jsonData=json_decode($_REQUEST['data']);
		$resultArray =$dbf->fetchOrder("state st,clients c,service s,technicians t,assign_tech at,work_order w","st.state_code=c.state AND c.id=w.client_id AND w.service_id=s.id AND w.work_status IN('Dispatched','In Progress','Completed','Ready to Invoice','Invoiced','Assigned','Scheduled') AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND t.id='".$jsonData->userid."'","w.id DESC","st.state_name,c.name,c.city,w.id,w.wo_no,w.work_status,s.service_name,at.start_date,at.start_time","");
		$dataArray = array();
		...
		foreach($resultArray as $res_JobBoard){
			...
			$startdatetime = ($res_JobBoard['start_date']<>'0000-00-00')? date("d-M-Y",strtotime($res_JobBoard['start_date'])).' '.$res_JobBoard['start_time']:""; 
