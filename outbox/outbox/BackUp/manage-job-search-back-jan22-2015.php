                        ...
                        <div class="spacer"></div>
                              <?php
								...
								if($_REQUEST['srchInputBox']!=''){
									...
									$sch=$sch."w.wo_no IN ($searchString) OR c.name IN ($searchString) OR c.email IN ($searchString) OR w.purchase_order_no IN ($searchString) OR w.pickup_phone_no IN ($searchString) OR w.pickup_address IN ($searchString) OR w.pickup_city IN ($searchString) OR w.work_status IN ($searchString) OR c.phone_no IN ($searchString) OR c.city IN ($searchString) OR c.address IN ($searchString) OR s.service_name IN ($searchString) OR cl.name IN($searchString) OR temp.techname IN($searchString) OR w.invoice_no IN ($searchString)";
									if($sch !=''){
