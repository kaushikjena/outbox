<?php 
	...
	$srchClientArr = is_array($_REQUEST['srchClient'])? $_REQUEST['srchClient']:(!is_array($_REQUEST['srchClient'])? explode(",",$_REQUEST['srchClient']):array());
?>
                ...
                <div class="rightcolumjobboard">
            		<div class="rightcoluminner">
							  ...
							  // echo $cond;exit;
                               ...
                               <!-----Table area start------->
                           		<table id="no-more-tables" class="draggable">
                                    ...
                                    <tbody>
                           <?php
								foreach($dbf->fetchOrder("state st,clients c,service s,technicians t,assign_tech at,work_order w",$cond,"w.id DESC LIMIT $startpoint,$perpage","st.state_name, c.city, c.name, s.service_name, w.wo_no, w.purchase_order_no, w.work_status, w.created_by,w.invoice_no, t.first_name,t.middle_name,t.last_name","")as  $res_JobBoard) {
								//get client name
                        ...
                        <!-----Table area end------->
                            ...
                            <?php }?>
                            <div  align="center"><?php if($num>0) { echo $dbf->Pages($num,$perpage,"admin_completed_job_report?srchClient=$creatorlist&srchTechnician=$_REQUEST[srchTechnician]&srchService=$_REQUEST[srchService]&FromDate=$_REQUEST[FromDate]&ToDate=$_REQUEST[ToDate]&");}?></div>
