                            ...
                            <form name="SrchFrm" id="SrchFrm" action="" method="post">
							  ...
							  function getStartAndEndDate($week, $year) {
								...
								foreach($week_array as $k=>$w){
									...
									foreach($dbf->fetchOrder("technicians t,assign_tech at,work_order w","at.wo_no=w.wo_no AND at.tech_id=t.id AND w.created_by<>0 AND w.work_status='Completed' AND at.start_date BETWEEN $date_range","","w.wo_no","")as $res){
							  ...
							  ###########################################################################
							   ...
							   //echo $sch;exit;
							   if($sch!=''){
								 $cond="c.id=w.created_by AND w.approve_status='1' AND w.work_status='Completed' AND w.created_by<>0 AND ".$sch;
							   }elseif($sch==''){
								 $cond="c.id=w.created_by AND w.approve_status='1' AND w.work_status='Completed' AND w.created_by<>0";
							   }
							   //echo $cond;
							  ?>
                            <!-----Table area start------->
