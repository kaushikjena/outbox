...
function print_doc(val){
 if(val=='print'){
	...
	if(x){
		$("#frmTech").attr("action","admin-technician-invoiced-payment-print");
	...
	if(x){
		$("#frmTech").attr("action","admin-technician-invoiced-payment-pdf");
		//$("#frmTech").attr("target","_blank");
		$("#frmTech").submit();
	}
 }
}
/**********Send invoiced email to Tech*********/
function sendInvoicedEmail(){
	var x = confirm("Are you sure you want to send invoiced email to tech.");
	var url="send-invoiced-email-technician.php";	
	var tid=$('#tid').val();
	var billperiod=$('#billperiod').val();
	var wonos=$('#wonos').val();
	if(x){
		$.post(url,{"choice":"send_email","wonos":wonos,"billperiod":billperiod,"tid":tid,"status":"invoiced"},function(res){ 
			//alert(res);
			$.fancybox.showActivity();
				//$.fancybox(res,{centerOnScroll:true,hideOnOverlayClick:false});
			 if(res=='1'){
				$.fancybox("<span style='color:#090;'><b>Email Send Successfully.</b></span>",{centerOnScroll:true,hideOnOverlayClick:false});
			 }else{			
				 $.fancybox("<span style='color:#F00;'><b>Email Sending failed.</b></span>",{centerOnScroll:true,hideOnOverlayClick:false});			
			 }
		});
	}else{
		return false;
         ...
         <div id="contentdiv">
                ...
                <div class="rightcolumjobboard">
            		<div class="rightcoluminner">
                        ...
                        <!-----Table area start------->
                         <!-----Billing div start--------->
                          <div  class="divBilling">
                            <div class="greenText" align="center">Tech Invoiced Bill Payment For Period <?php echo $_REQUEST['billperiod'];?></div>
                            ...
                            <div  class="textboxbillview">Invoiced</div>
                            <div  class="formtextaddbilllong">Bill Period :</div>
                            <div  class="textboxbillview"><?php echo $_REQUEST['billperiod'];?></div>
                        ...
                        <input type="hidden" name="billperiod" id="billperiod" value="<?php echo $_REQUEST['billperiod'];?>"/>
                        <input type="hidden" name="wonos" id="wonos" value="<?php echo $_REQUEST['wonos'];?>"/>
                        <div align="center">
                        <input type="button" class="buttonText" value="Return Back" onClick="window.location='manage-technician-invoiced-payments'"/> 
                        ...
                        <input type="button" class="buttonText" value="Send Email" onClick="sendInvoicedEmail();"/>
                        </div>
                        </form>
                        <div class="spacer"></div>
                       <!-----Table area end------->
