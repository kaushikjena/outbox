<?php 
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='cancel'){	
		...
		###########Track user activity in work order notes table#############
		header("Location:manage-job-board-completed");exit;
	}
	###########Cancel record from work order Table###############
                            ...
                            <input type="hidden" name="page" value="CompletedBoard"/>
                              ...
                              <!-----Table area start------->
                                <table id="no-more-tables" class="draggable">
								    ...
								    <?php
									$resArray=$dbf->fetchOrder("state st,clients c,service s,technicians t,assign_tech at,work_order w","t.id=$sgRes[techid] AND " .$cond,$orderby." ".$orderType,"st.state_name, c.name,c.city, w.id,w.wo_no,w.service_id,w.work_status,w.created_by,w.parts_status, s.service_name, t.first_name, t.middle_name, t.last_name, at.start_date, at.start_time",$groupby);
									//print "<pre>";
									//print_r($resArray);
