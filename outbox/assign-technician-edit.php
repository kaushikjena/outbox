...
<?php }else if(isset($_REQUEST['choice'])&& $_REQUEST['choice']=="data_insert"){
	...
	if($num>0){
		$oldTechid =$dbf->getDataFromTable("assign_tech","tech_id","wo_no='".$WorkNo."'");
		//update work order table
		if($asgnStartDate !=''){
			$dbf->updateTable("work_order","work_status='Scheduled', schedule_status='Scheduled', reschedule_status=0, updated_date=now(), updated_by='$_SESSION[userid]'","wo_no='$WorkNo'");
		}else{
			//update work order table
		...
		/********Client Email************/
		/********Old Technician Email about change assignment************/
		if($oldTechid != $Techid){
			//get technician details
			$oldtotech=$dbf->strRecordID("technicians","id, first_name, middle_name, last_name, email, contact_phone, pay_grade, sms_alert","id='".$oldTechid."'");
			$TechName=$oldtotech['first_name'].'&nbsp;'.$oldtotech['middle_name'].'&nbsp;'.$oldtotech['last_name'];
			$oldTechPhoneNo=$oldtotech['contact_phone'];
			$res_template=$dbf->fetchSingle("email_template","id='25'");
			$from=$res_template['from_email'];
			$from_name=$res_template['from_name'];
			$subject=$res_template['subject']."==".$clientName."==".$WorkNo;
			$input=$res_template['message'];
			//get technician details
			$oldto=$oldtotech['email'];
			$email_body=str_replace(array('%TechName%','%WorkOrders%','%PurchaseOrder%'),array($TechName,$WorkNo,$purchaseOrder),$input);
			$headers = "MIME-Version: 1.0\n";
			$headers .= "Content-type: text/html; charset=UTF-8\n";
			$headers .= "From:".$from_name." <".$from.">\n";
			//echo $email_body;exit;
			@mail($oldto,$subject,$email_body,$headers);
		}
		/********Old Technician Email about change assignment************/
		#########Email Sending End#################
		################APP Notification ##########################
		...
		if($clientName=='Amazon'){
			$msgtemplate=$dbf->getDataFromTable("sms_template","message","client_id='".$resOrder['created_by']."'");
			if($msgtemplate !=""){
			...
			//send sms if the sms_alert is on for that tech
			  ...
			  ###########################################################################
			  ######################MESSAGE FORWARD TO OLD TECH##########################
			  if($oldTechid != $Techid){
				  if($oldtotech['sms_alert']=='1'){
					  $oldmsgtemplate=$dbf->getDataFromTable("sms_template","message","id='2'");
					  $oldtxaMessage=str_replace(array('%WorkOrder%'),array($WorkNo),$oldmsgtemplate);
					  $OLDTechPhoneNo = preg_replace('/[\. -]/', '', $oldTechPhoneNo);
					  $tosendOldtech="1".$OLDTechPhoneNo;
						try{
							$api = new TextMagicAPI(array(
								"username" => TEXT_MAGIC_API_USER,
								"password" => TEXT_MAGIC_API_PASSWORD
							...
							//$phones = array(99912345678);
							$oldtechphones = array($tosendOldtech);
							$results = $api->send($oldtxaMessage, $oldtechphones, true);
							//your data base code to insert; 
					   }catch(Exception $e){
					   }
				  }
