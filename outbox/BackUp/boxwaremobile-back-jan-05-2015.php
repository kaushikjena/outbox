<?php
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="dashboard" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		foreach($resultArray as $res_JobBoard){
			$subArray = array("woid" => $res_JobBoard['id'],"wo#" => $res_JobBoard['wo_no'],"ServiceName" => $res_JobBoard['service_name'],"CustomerName" => $res_JobBoard['name'],"Latitude" => $res_JobBoard['latitude'],"Longitude" => $res_JobBoard['longitude'],"AssignedDate" => $res_JobBoard['assign_date']);
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="jobdetails" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		$assignedDate = ($resTech<>'' && $resTech['assign_date']<>'0000-00-00')? date("d-M-Y",strtotime($resTech['assign_date'])):"";
		$startDate = ($resTech<>'' && $resTech['start_date']<>'0000-00-00')?  date("d-M-Y",strtotime($resTech['start_date'])).' '.$resTech['start_time'] :"";
		...
		$res_woservice = $dbf->fetch("equipment e,work_type wt,workorder_service ws","e.id=ws.equipment AND wt.id=ws.work_type AND ws.workorder_id='".$jsonData->woid."'");
		  foreach($res_woservice as $arrWorkservice){
			...
			$subArray = array("Work Type" => $arrWorkservice['worktype'],"Equipment" => $arrWorkservice['equipment_name'],"Model" => $arrWorkservice['model'],"Quantity" => $arrWorkservice['quantity'],"Tech Price" => "$".number_format($TechPrice,2),"Total Price" => "$".number_format($total,2));
			 array_push($dataArray,$subArray);
		}
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="servicecall" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		foreach($resultArray as $res_JobBoard){
			...
			$startdatetime = $res_JobBoard['start_date']? date("d-M-Y",strtotime($res_JobBoard['start_date'])).' '.$res_JobBoard['start_time']:""; 
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="submitschedule" && $_SERVER['REQUEST_METHOD']=='GET'){
		...
		if($num>0){
			...
			if($asgnStartDate !=''){
				$dbf->updateTable("work_order","schedule_status='Scheduled'","wo_no='$WorkNo'");
			}	
			//update assign_tech table
			$string="start_date='$asgnStartDate', start_time='$asgnStartTime',end_time='$asgnEndTime'";
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="submitWorkstatus"){
		...
		if($_POST['fileSignature']){
			$filenamesign=$jsonData->wono.'_'.time().'_fileSignature.png';
			$worksignpath="mobile_image/signature/";
			file_put_contents($worksignpath.$filenamesign,base64_decode($_POST['fileSignature']));
			$stringData.=",tech_signature='$filenamesign'";
		}
		//for work image
		if($_POST['fileWorkImage']){
			$file_name=$jsonData->wono.'_'.time()."_fileWorkImage.png";
			$workimgpath="mobile_image/workimage/";
			file_put_contents($workimgpath.$file_name,base64_decode($_POST['fileWorkImage']));
			$stringData.=",work_image='$file_name'";
		}
	...
	if(isset($_REQUEST['method']) && $_REQUEST['method']=="jobreport" && $_SERVER['REQUEST_METHOD']=='GET'){
		//print_r(json_decode($_REQUEST['data']));exit;
		$jsonData=json_decode($_REQUEST['data']);
		$resultArray =$dbf->fetchOrder("state st,clients c,service s,technicians t,assign_tech at,work_order w","st.state_code=c.state AND c.id=w.client_id AND w.service_id=s.id AND w.work_status='Assigned' AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND t.id='".$jsonData->userid."'","w.id DESC","st.state_name,c.name,c.city,w.id,w.wo_no,w.work_status,w.pickup_city,w.pickup_state,s.service_name,at.start_date","");
		$dataArray = array();
