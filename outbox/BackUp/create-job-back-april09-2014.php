...
if(isset($_REQUEST['action']) && $_REQUEST['action']=="insert" && $_SERVER['REQUEST_METHOD']=='POST'){
	$PurchaseOrder=mysql_real_escape_string($_POST['PurchaseOrder']);
	...
	if($numuser>0){
		...
		$string="name='$CustomerName', user_type='client', email='$CustomerEmailID', contact_name='$CustomercontactName', address='$CustAddress', city='$CustCity', state='$CustState',fax_no='$_REQUEST[Faxno]',zip_code='$_REQUEST[CustZip]', phone_no='$_REQUEST[CustPhone]', alt_phone='$_REQUEST[altCustPhone]', created_date=now()";
		...
		$CustCity=mysql_real_escape_string($_REQUEST['CustCity']);
		$CustState=mysql_real_escape_string($_REQUEST['CustState']);
		$string="name='$CustomerName', user_type='client', email='$CustomerEmailID', contact_name='$CustomercontactName', address='$CustAddress', city='$CustCity', state='$CustState',fax_no='$_REQUEST[Faxno]',zip_code='$_REQUEST[CustZip]', phone_no='$_REQUEST[CustPhone]', alt_phone='$_REQUEST[altCustPhone]', status=0, created_date=now()";
		...
		if($_REQUEST['techNotes']<>''){
			...
			$string5="workorder_id=$woid, user_type='$_SESSION[usertype]', user_id='$_SESSION[userid]', wo_notes='$techNotes', created_date=now()";
			$dbf->insertSet("workorder_notes",$string5);
		}
		###########Insert Into work order notes table#############
                ...
                <div class="rightcolumjobboard">
            		<div class="rightcoluminner">
                        ...
                        <div class="headerbg">
                            ...
                            <div style="float:right;"><input type="submit" name="submitbtn" id="submitbtn" class="buttonText2" value="Submit Form"/>
								<?php if($_REQUEST['src']=='disp'){?>
                                   <a href="manage-job-board-dispatch" style="text-decoration:none;"><input type="button" class="buttonText2" value="Back" tabindex="30" onClick="document.location.href='manage-job-board-dispatch'"/></a>
                                <?php }else{ ?>
                                   <a href="manage-job-board" style="text-decoration:none;"><input type="button" class="buttonText2" value="Back" tabindex="30" onClick="document.location.href='manage-job-board'"/></a>
                                <?php } ?>
                             </div>
                        </div>
                        ...
                        <!-----Table area start------->
                            ...
                            <div  class="textboxcjob">
                               <input type="text" class="textboxjob" name="ScheduledDate" id="ScheduledDate" readonly value="<?php if(!empty($resTech)){echo date("d-M-Y",strtotime($resTech['start_date']));}?>">
                            ...
                            <div  class="divPickup">
                             ...
                             <div><textarea name="adminNotes" id="adminNotes"  class="textareaOrder" tabindex="24"></textarea><br/><label for="adminNotes" id="lbladminNotes" class="redText"></label></div>
                             <div class="spacer" style="height:10px;"></div>
                            </div>
                            <!-----admin note div end--------->
                            <!-----tech note div start--------->
                            <div  class="divNotes">
                             ...
                             <div><textarea name="techNotes" id="techNotes"  class="textareaOrder" tabindex="25"></textarea><br/><label for="techNotes" id="lbltechNotes" class="redText"></label></div>
                             <div class="spacer" style="height:10px;"></div>
                            </div>
                            <!-----tech note div end--------->
                            <div class="spacer"></div>
