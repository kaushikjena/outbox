...
if(isset($_REQUEST['action']) && $_REQUEST['action']=="insert" && $_SERVER['REQUEST_METHOD']=='POST'){
	...
	//fetch data from assign tech table
	$resTech = $dbf->fetchSingle("assign_tech ","wo_no='$fetchArray[wo_no]'");
	...
	if($woid){
		...
		###########Track user activity in work order notes table#############
		###########Insert Into assign tech table#############
		if(!empty($resTech)){
			//insert into assign_tech table
			$string1="wo_no='$WorkOrder',tech_id='$resTech[tech_id]',  assign_date='$resTech[assign_date]',start_date='',created_date=now()";
			$insassign =$dbf->insertSet("assign_tech",$string1);
		}
		###########Insert Into assign tech table#############
	}
	echo "1";exit;
