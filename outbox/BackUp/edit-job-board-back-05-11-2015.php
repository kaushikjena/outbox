...
if(isset($_REQUEST['action']) && $_REQUEST['action']=="update" && $_SERVER['REQUEST_METHOD']=='POST'){
	...
	$string1="wo_no='$_REQUEST[WorkOrder]', purchase_order_no='$PurchaseOrder',   service_id='$_REQUEST[cmbService]',client_id='$clientid', pickup_location='$Pickuplocation', pickup_address='$Address', pickup_city='$City', pickup_state='$State', pickup_zip_code='$_REQUEST[Zip]', pickup_phone_no='$_REQUEST[Phone]', pickup_alt_phone='$_REQUEST[altPhone]',tracking_number='$_REQUEST[track_no]',carrier_company='$_REQUEST[carrier_company]', work_status='$_REQUEST[cmbStatus]', notes='$notes', created_by='$_REQUEST[cmbClient]', updated_date=now(), updated_by='$_SESSION[userid]',invoice_no='$InvoiceNo', invoice_no_update_date=now(), invoice_no_status=1";
...
function sendEmailUser(){
	...
	var wono=$('#wono').val();
	if(x){
	 	$.post(url,{"choice":"send_email","wono":wono,"woid":woid,"uname":uname,"uemail":uemail,"fromemail":fromemail,"fromname":fromname,"subject":subject,"message":message},function(res){ //alert(res);
		 if(res=='1'){
			$.fancybox("Email Send Successfully",{centerOnScroll:true,hideOnOverlayClick:false});
		 }else{			
			 $.fancybox("Email Sending failed",{centerOnScroll:true,hideOnOverlayClick:false});			
                    ...
                    <form name="createJob" id="createJob" action="" method="post" onSubmit="return validate_createjob();" autocomplete="off" enctype="multipart/form-data">
                        <div class="headerbg">
                            ...
                            <div style="float:right;">
								...
								<?php if($_REQUEST['src']=='disp'){$path="manage-job-board-dispatch?g=$_REQUEST[hidk]";}else{$path="manage-job-board";} ?>
                        ...
                        <!-----Table area start------->
                            ...
                            <?php if($resTech<>''){
								if($res_editJobBoard['work_status']=='Assigned'){?>
                           ...
                           <div class="spacer" style="height:12px;"></div>
                            ...
                            <div align="center">
                                ...
                                <input type="button" class="buttonText3" value="Notification To User" tabindex="40" onClick="send_email_user('<?php echo $res_editJobBoard['id'];?>','<?php echo $res_editJobBoard['wo_no'];?>');"/>
