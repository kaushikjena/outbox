                            ...
                            <form name="SrchFrm" id="SrchFrm" action="" method="post">
							  ...
							  function getStartAndEndDate($week, $year) {
								...
								foreach($week_array as $k=>$w){
									...
									foreach($dbf->fetchOrder("work_order w,work_order_tech wt","w.wo_no=wt.wo_no AND w.created_by<>0 AND wt.work_status='Completed' AND wt.arrival_date BETWEEN $date_range","","wt.wo_no","")as $res){
							  ...
							  ###########################################################################
							   ...
							   //echo $sch;exit;
							   if($sch!=''){
								 $cond="c.id=w.created_by AND w.job_status='Assigned' AND w.approve_status='1' AND w.work_status='Completed' AND w.created_by<>0 AND ".$sch;
								  // echo $cond;exit;
							   }
							   elseif($sch==''){
								 $cond="c.id=w.created_by AND w.job_status='Assigned' AND w.approve_status='1' AND w.work_status='Completed' AND w.created_by<>0";
							   }
							   //echo $cond;
							  ?>
                            <!-----Table area start------->
