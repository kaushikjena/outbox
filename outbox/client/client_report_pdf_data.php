...
<body>
	  <?php 
	   ...
	   //echo $sch;exit;
	   if($sch!=''){
		  $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND work_status='Assigned' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND w.created_by='$uid' AND ".$sch;
	   }
	   elseif($sch==''){
		  $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND work_status='Assigned' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND w.created_by='$uid'";
	   }	
	   //print $cond;exit;
