...
<?php
	...
	if($_REQUEST['id']<>''){ 
		...
		$dbf->updateTable("work_order","work_status='WFP',updated_date=now()","id='$_REQUEST[id]'");
		//get purchase order
		$purchaseOrder=$res_wo['purchase_order_no'];
		//email to admin
		$admin_notification = $dbf->getDataFromTable("admin_email_notification","status","id=8");
	    $admin_email = $dbf->getDataFromTable("admin_email_notification","to_email","id=8");
