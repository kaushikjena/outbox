...
function Search_Records(){
	$("#SrchFrm").attr("action", "tech_report");
	$("#SrchFrm").submit();
}
...
function print_doc(val,page){
 if(val=='word'){
	$("#SrchFrm").attr("action", "tech_report_word");
	$("#SrchFrm").submit();
 }else if(val=='excell'){
	$("#SrchFrm").attr("action", "tech_report_excell");
	$("#SrchFrm").submit(); 
 }else if(val=='pdf'){
	$("#SrchFrm").attr("action", "tech_report_pdf");
	$("#SrchFrm").submit(); 
 }else if(val=='print'){
	$("#SrchFrm").attr("action", "tech_report_print?page="+page);
                        ...
                        <div id="contenttable">
                        	<div style="width:100%;float:left;">
                              ...
                              <div style="margin-bottom:5px;" align="center">
                                   ...
                                   <div class="textboxcsrch">
                                     <select name="srchCust" id="srchCust" class="selectboxsrch">
                                            ...
                                            <?php foreach($dbf->fetchOrder("work_order wo,clients cl","wo.client_id=cl.id AND wo.work_status='Assigned'","cl.id DESC","","cl.name")as $customer){?>
                              ...
                              <?php
							   ...
							   //echo $sch;exit;
							   if($sch!=''){
								 $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND t.id='$_SESSION[userid]' AND ".$sch;
							  // echo $cond;exit;
							   }
							   elseif($sch==''){
								 $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND t.id='$_SESSION[userid]'";
							   }
							   //Pagination 
                              ...
                              <!-----Table area start------->
                                <table id="no-more-tables" class="draggable">
                                    ...
                                    <td data-title="CreatedDate"><?php echo ($res_JobBoard['start_date']<>'0000-00-00')?date("d-M-Y",strtotime($res_JobBoard['start_date'])):'';?></td>                         
                            ...
                            <?php }?>
                             <div  align="center"><?php if($num>0) { echo $dbf->Pages($num,$perpage,"tech_report?srchCust=$_REQUEST[srchCust]&srchService=$_REQUEST[srchService]&FromDate=$_REQUEST[FromDate]&ToDate=$_REQUEST[ToDate]&");}?></div>
                          </div>
                        </div>
                        <div class="spacer"></div>
                    </div>
