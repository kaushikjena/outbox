...
<body>
	 <?php $sch="";
       ...
       if($_REQUEST['hidaction'] == 1){
           ...
           //echo $sch;exit;
           if($sch!=''){
			 $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND w.work_status='Assigned' AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND t.id='$uid' AND ".$sch;
		  // echo $cond;exit;
		   }
		   elseif($sch==''){
			 $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND w.work_status='Assigned' AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id AND t.id='$uid'";
		   }	
          // print $cond;exit;
       ...
       <table height="61" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#808080" width="100%" style="border:solid 1px #999;"> 
        <thead>
          <tr bgcolor="#E6F9D5">
           <th width="6%" height="27" align="left" valign="middle" class="fetch_headers">WO No</th>
            ...
            <th width="8%" height="27" align="left" valign="middle" class="fetch_headers">DeliveryAddress</th>
            <th width="10%" height="27" align="left" valign="middle" class="fetch_headers">DeliveryZipcode</th>
		...
		?>
          <tr>
            ...
            <td height="25" align="left" valign="middle" class="fetch_contents"><?php if($res_JobBoard['assign_date']!='0000-00-00'){echo date("d-M-Y",strtotime($res_JobBoard['assign_date']));}?></td>
            ...
            <td height="25" align="left" valign="middle" class="fetch_contents"><?php echo $res_JobBoard['address'];?></td>
            <td height="25" align="left" valign="middle" class="fetch_contents"><?php echo $res_JobBoard['zip_code'];?></td>
