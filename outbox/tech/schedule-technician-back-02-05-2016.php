...
<?php }else if(isset($_REQUEST['choice'])&& $_REQUEST['choice']=="data_update"){
	...
	if($num>0){
		...
		if($admin_notification==1){
			...
			$email_body=str_replace(array('%AdminName%','%TechName%','%StartDate%','%StartTime%','%EndTime%','%WorkOrders%','%PurchaseOrder%'),array($AdminName,$TechName,$_REQUEST['StartDate'],$_REQUEST['StartTime'],$_REQUEST['EndTime'],$WorkNo,$purchaseOrder),$input);
