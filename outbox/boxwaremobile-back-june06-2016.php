			...
			if($num>0){
				...
				if($res_login['status']=='1'){
					...
					if($res_login['user_type']=='tech'){
						...
						$dataArray=array();
						$resArray = $dbf->fetchOrder("assign_tech at,clients c,work_order wo"," at.wo_no=wo.wo_no AND wo.client_id=c.id AND at.tech_id='".$res_login['id']."'","wo.id","at.start_date,at.start_time,wo.id,wo.purchase_order_no,at.wo_no,c.name,c.city,c.latitude,c.longitude");
						foreach($resArray as $resevent) {
							...
							$subArray = array("Title" => $title,"Description" => $description,"StartDate" => $eventdt,"Latitude" => $resevent['latitude'],"Longitude" => $resevent['longitude']);
