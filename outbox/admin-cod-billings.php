...
if($_REQUEST['action']=='payment'){
	    //Get Purchase Order 
		$purchase_order=$dbf->getDataFromTable("work_order","purchase_order_no","wo_no='$res_viewJobBoard[wo_no]'");
		
		$codname=$res_viewJobBoard['name'];
		$toemail=$res_viewJobBoard['email'];
		...
		$Url=$res_admin['site_url']."/cod-checkout.php?cid=".$res_viewJobBoard['client_id']."&wono=".$res_viewJobBoard['wo_no'];
		$activateUrl='<a href="'.$Url.'" target="_blank">Click Here</a>';
		
		$email_body=str_replace(array('%ClientName%','%WoNo%','%PurchaseOrder%','%ServiceName%','%CompletedDate%','%TechnicianName%','%Subtotal%','%ActivationLink%'),array($codname,$res_viewJobBoard['wo_no'],$purchase_order,$servname,$compdate,$techname,number_format($_REQUEST['amount'],2),$activateUrl),$input);
...
<script>
function PayOffline(){
	document.frmClient.action='cod-offline-payment.php';
            		...
            		<div class="rightcoluminner">
                        ...
                        <!-----Table area start------->
                           ...
                           <div class="spacer" style="height:12px;"></div>
                            ...
                            <input type="hidden" name="cid" value="<?php echo $res_viewJobBoard['client_id'];?>"/>
                            <input type="hidden" name="wid" value="<?php echo $res_viewJobBoard['id'];?>"/>
                            <input type="hidden" name="wono" value="<?php echo $res_viewJobBoard['wo_no'];?>"/>
                            <div align="center">
                         	 <input type="button" class="buttonText" value="Return Back" onClick="window.location='manage-cod-billings'"/> 
                             <input type="submit" class="buttonText" value="Send To COD"/>
                             <input type="button" class="buttonText" value="Offline Payment" onClick="PayOffline();"/> 
                             </div>
                             </form>
                          	<div class="spacer"></div>
                           <!-----Table area end------->
