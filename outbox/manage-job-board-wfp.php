<?php 
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action']=='cancel'){	
		...
		###########Track user activity in work order notes table#############
		header("Location:manage-job-board-wfp");exit;
	}
	###########Cancel record from work order Table###############
...
function ClearFields(){
	$('#FromDate').val("");
	$('#ToDate').val("");
	$.post("unset-session.php",{"src":"wfp"},function(res){
		$("#schaction").val("");
		$("#SrchFrm").submit();
	});
}
...
function edit_filter(){
	window.location.href="edit-filter-search-wfp";
}
/*********Function to show create job************/
...
/**********Send Notification to assigned Tech*********/
	...
	<form name="frmRedirect" id="frmRedirect" action="" method="post"> 
        ...
        <input type="hidden" name="src" value="wfp"/>
                ...
                <div class="rightcolumjobboard">
            		<div class="rightcoluminner">
                    	<?php
							...
							$srchClient=$_SESSION['requestw']['search']['srchClient'];
							$srchTechnician=$_SESSION['requestw']['search']['srchTechnician'];
							$srchService=$_SESSION['requestw']['search']['srchService'];
							$srchStatus=$_SESSION['requestw']['search']['srchStatus'];
							$srchJobStatus=$_SESSION['requestw']['search']['srchJobStatus'];								
						   ...
						   if($sch!=''){
							 ...
							 $cond="c.state=st.state_code AND c.id=w.client_id AND w.service_id=s.id AND w.work_status ='WFP' AND w.approve_status='1' AND w.wo_no=at.wo_no AND at.tech_id=t.id";
						   }
						   //condition for users
						   if($implode_clients <>''){
